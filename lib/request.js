class Request {
   ...
   */
   ...
   get apiGatewayRouteKey() {
     ...
     if (this.event?.pathParameters?.proxy) {
       routeKey = routeKey.replace("proxy+", this.event.pathParameters.proxy);
       routeKey = routeKey.replace("{", "");
       routeKey = routeKey.replace("}", "");
     }
     let route = this.#splitRoute(routeKey);
     return route.substr(0, route.lastIndexOf('.')) || route;
   }
