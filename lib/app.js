...
class App {
   ...
   * @param {string} [defaultHandler=null]
   ...
   * }
   */
  constructor(event, context, fileExtension = "js", defaultHandler = null) {
    let request = new Request(event);
    return new Promise((resolve, reject) => {
      let response = new Response(resolve, reject);
      this.#route(request, response, fileExtension, defaultHandler);
    });
  }
   ...
   * @private
   */
  async #route(req, res, fileExtension, defaultHandler) {
    try {
      let File;
      if (defaultHandler !== null) {
        File = await import(`${process.cwd()}/${defaultHandler}.${fileExtension}`);
      } else {
        File = await import(`${process.cwd()}/${req.file}.${fileExtension}`);
      }
