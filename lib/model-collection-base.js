...
class ModelCollectionBase {
  ...
  */
   ...
   * @param {Object} [options={}] - Fetch options, extended with custom fields.
   * @param {string} [options.method='GET'] - HTTP method (used for caching decisions).
   * @param {number} [options.lockTimeoutMs] - Optional lock timeout in milliseconds.
   * @param {number} [options.cacheMs] - Optional cache duration in milliseconds (only used for GET).
   * @param {number} [options.abortTimeoutMs] - Optional abort timeout in milliseconds.
   * @returns {Promise<Response>} The network or cached response.
   * @throws {Error} If the fetch is aborted or a lock times out.
   */
  static async fetch(url, options = {}) {
    const opts = { ...options };
    const method = (opts.method || 'GET').toUpperCase();
    const shouldLock = !!opts.lockTimeoutMs;
    const isCacheable = method === 'GET' && !!opts.cacheMs;
    ...
    try {
      if (isCacheable) {
        ...
        if (this._isValidResponse(cached)) {
          return cached.clone();
        }
      }

      response = await this._fetchWithAbort(url, opts, abortTimeoutMs);

      if (isCacheable) {
        if (response.ok) {
          this.setCache(url, response.clone(), opts.cacheMs);
        } else {
          this.clearCache(url);
        }
      }
