class Request {
  ...
  get type() {
    if (this.event.Records) {
      ...
      return 'aws:cognito';
    } else if (this.event.source && this.event?.['detail-type'] === 'Scheduled Event') {
   ...
   */
  get #eventKey() {
    let resources = this.event.resources[0];
    let splits = resources.split(':');
    let rule = splits[splits.length - 1];
    let ruleSplits = rule.split('/');
    let ruleToFile = ruleSplits[1].replace(/\.?([A-Z]+)/g, (_x,y) => "-" + y.toLowerCase()).replace(/^-/, '')
    return ruleToFile;
  }
